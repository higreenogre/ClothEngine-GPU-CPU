...
public class ClothPlane6 : MonoBehaviour
{
            ...
            for ( int x = 0; x < tricount; x++)
            {
                for ( int y = 0; y < gridLength; y++)
                {
                    if (y != triangles[3 * x + 0] && y != triangles[3 * x + 1] && y != triangles[3 * x + 2])
                    {
                        ...
                        n = Vector3.Normalize(Vector3.Cross(p3 - p2, p4 - p2));
                        ...
                        n = h < 0 ? -n : n;
                        float f = Vector3.Dot(p1 - p2, n) - 0.5f;
                        ...
                        if ((hlast / h < 0f) && (howfar1.magnitude <= 0.75 * maxstiff) && (howfar2.magnitude <= 0.75 * maxstiff) && (howfar3.magnitude <= 0.75 * maxstiff))// && (hlast1 > -1f) && (hlast2 > -1f) && (hlast3 > -1f))   //&& (howfar.magnitude < maxstiff) && (howfar1.magnitude <= maxstiff) && (howfar2.magnitude <= maxstiff) && (howfar3.magnitude <= maxstiff) 
                        {
                            detectioncount++;
                            ...
                            pos[y] -= times * weights[y] * h * delp1 / denom;
                            pos[triangles[3 * x + 0]] -= times * weights[triangles[3 * x + 0]] * h * delp2 / denom;
                            pos[triangles[3 * x + 1]] -= times * weights[triangles[3 * x + 1]] * h * delp3 / denom;
                            pos[triangles[3 * x + 2]] -= times * weights[triangles[3 * x + 2]] * h * delp4 / denom;
